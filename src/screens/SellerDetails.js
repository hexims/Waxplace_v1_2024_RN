...
import {createCorreosPreRegistration} from '../actions/_CorreosAPI';
...
const SellerDetails = ({props, route, userReducer, updateUserDetails}) => {
  ...
  const [postalCode, setPostalCode] = useState(
    route?.params?.item?.albumId?.postalCode,
  );
  ...
  const acceptOrder = async () => {
    ...
    try {
      ...
      let sellerDetails = {
        'prer:Identificacion': {
          'prer:Nombre': firstName,
          'prer:Apellido1': lastName,
          'prer:Empresa': firstName,
          'prer:PersonaContacto': firstName + ' ' + lastName,
        },
        'prer:DatosDireccion': {
          'prer:Direccion': deliveryAddress,
          'prer:Localidad': city,
          //   'prer:Provincia': userReducer?.profileDetails?.city,
        },
        'prer:CP': postalCode,
        'prer:Telefonocontacto': mobileNumber.toString(),
        'prer:Email': email,
        'prer:DatosSMS': {
          'prer:NumeroSMS': mobileNumber.toString(),
          'prer:Idioma': '1',
        },
      };

      let isCustomsCorreosShipmentValue = false;
      if (
        isCustomsCorreosShipment(
          postalCode,
        ...
        isCustomsCorreosShipmentValue = true;
        sellerDetails['prer:Identificacion']['prer:Nif'] = taxNumber;
      }
          ...
          'soapenv:Body': {
            'prer:PreregistroEnvio': {
              ...
              'prer:Envio': {
                ...
                'prer:Aduana': isCustomsCorreosShipmentValue
                  ? {
                      'prer:TipoEnvio': '6',
                      'prer:Factura': 'S',
        ...
        .then(result => {
          if (result.status == 'success') {
            ...
            addShipmentDetails(jsonData)
              .then(result => {
                if (result.status == 'success') {
                  route?.params?.callback(route?.params?.index, {
                    ...item,
                    shipmentDetails,
                    pickupDetails,
                    sellerInformation:{...item?.sellerInformation,zipcode:postalCode},
                    orderStatus: 'Order Accepted',
                  });
  ...
  return (
    <SafeAreaView style={styles.maincontainer(colors)}>
      <SearchBar
        ...
        searchText={''}
        searchFunction={text => {}}
        placeHolderCondition={'Seller Information'}
        showCart={false}
      />
      <ScrollView
...
export default connect(mapStateToProps, {updateUserDetails})(SellerDetails);

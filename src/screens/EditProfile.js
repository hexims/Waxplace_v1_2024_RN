...
const Address = ({props, route, userReducer, updateUserDetails}) => {
    ...
    let jsonData = {
      ...
      countryCode: phoneInput?.current?.getCountryCode(
        mobileNumber.includes('+')
          ? mobileNumber
          : userReducer?.profileDetails?.countryCode,
      ),
      countryCodeNumber: mobileNumber.includes('+')
        ? mobileNumber.split(value)[0]
        : userReducer?.profileDetails?.countryCodeNumber,
  ...
  const handleRegistration = async () => {
    ...
    if (mobileNumber == '' || mobileNumber !== verifiedMobile) {
      toast.show(textContent.EditProfile.alert_mobile_number);
      return;
    }

    if (taxNumber !== '' && !ValidateSpanishID(taxNumber).valid) {
      toast.show(textContent.EditProfile.alert_tax_number);
      return;
    }
    ...
    if (shareCodeError) {
      toast.show(textContent.EditProfile.alert_share_code);
      return;
    }
    if (shareCode !== '') {
      try {
        ...
        if (shareCode == userReducer?.profileDetails?.shareCode) {
  ...
  const confirmMobileOtpValidation = mobileOtp => {
    ...
    if (mobileOtp.length === 6) {
      ...
      compareTwilloOTP({
        ...
        .catch(err => {
          setMobileOtp('');
          toast.show(textContent.EditProfile.invalid_otp);
          updateUserDetails({prop: 'spinner', value: false});
        });
    } else {
      toast.show(textContent.EditProfile.otp_continue);
      updateUserDetails({prop: 'spinner', value: false});
    }
  };
  ...
  return (
    <SafeAreaView style={styles.maincontainer(colors)}>
      <SearchBar
        ...
        searchText={''}
        searchFunction={text => {}}
        placeHolderCondition={textContent.EditProfile.edit_profile}
        showCart={false}
      />
      <ScrollView
        ...
        <View
          ...
          <Text
            ...
            {textContent.EditProfile.share_code}
          </Text>
          <View
            style={{
              width: '100%',
            ...
            <View style={{flex: 4, height: '100%'}}>
              <TextInput
                ...
                onChangeText={value => setShareCode(value)}
                blurOnSubmit={false}
                placeholderTextColor={colors.primaryTextColor}></TextInput>
            </View>
          </View>
          {shareCodeError ? (
            <Text
              ...
              {textContent.EditProfile.share_code_error}
            </Text>
          ) : null}
          <View
            style={{
...
export default connect(mapStateToProps, {updateUserDetails})(Address);

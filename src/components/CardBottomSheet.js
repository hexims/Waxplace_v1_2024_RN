...
import {getOrderIdDetails} from '../actions/PurchaseFlowAPI';
...
export const CardBottomSheet = ({
  ...
  const handleSocketNotFired = () => {
    if (userReducer?.transactionId) {
      let transactionId = userReducer?.transactionId;

      updateUserDetails({prop: 'transactionId', value: null});
      checkPayInStatus(transactionId)
        .then(result => {
          if (result.status === 200 && result?.data) {
            const data = JSON.parse(result?.data?.Tag);

            if (data.type === 'subscription') {
              handleSubscriptionStatus(result, transactionId);
            } else if (data.type === 'purchaseToken') {
              handlePurchaseTokenStatus(result, transactionId);
            } else if (data.type === 'purchaseAlbum') {
              handlePurchaseAlbumStatus(result, transactionId);
            }
          } else {
        ...
        .catch(err => {
          setSpinner(false);
          toast.show('Payment has been cancelled by user.\nPlease try again');
        });
    }
  };
  ...
  const handleSubscriptionStatus = result => {
    if (result.data.Status === 'SUCCEEDED') {
      ...
      toast.show('User has been successfully subscribed');
      setSpinner(false);
    } else {
      toast.show('User subscription has failed');
      setSpinner(false);
    }
  };
  ...
  const handlePurchaseTokenStatus = result => {
    if (result.data.Status === 'SUCCEEDED') {
      ...
      toast.show('Token Purchase has been successful');
      profileDetails()
        .then(result => {
          if (result?.user) {
            updateUserDetails({
      ...
      toast.show('Token Purchase has failed');
      setSpinner(false);
    }
  };

  const handlePurchaseAlbumStatus = (result, transactionId) => {
    if (result.data.Status === 'SUCCEEDED') {
      getOrderIdDetails(transactionId)
        .then(response => {
          updateUserDetails({prop: 'currentOrderId', value: null});

          if (
            response?.status == 'success' &&
            response?.message !== 'Order Is Not Available'
          ) {
            updateUserDetails({
              prop: 'profileDetails',
              value: response?.user,
      ...
      toast.show('Abum Purchase has failed');
      setSpinner(false);
    }
  };
  ...
  useEffect(() => {
    if (userReducer?.transactionId && userReducer?.isConnectedToInternet) {
      handleSocketNotFired();
      setSpinner(true);
    }
  }, [userReducer?.transactionId, userReducer?.isConnectedToInternet]);
    ...
    try {
      ...
      switch (paymentType) {
        ...
        case 'purchaseToken':
          ...
          Tag.tokens = amount;
          response = await directPayment(userReducer, amount, CardId, Tag);
          break;
    ...
    <View
      ...
      <View
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (spinner) {
                  Alert.alert(
                    '',
                    'Payment is in processing. Cancelling can cause issues to current Payment\nDo wish to continue?',
                    [
                      {
        ...
        <View
          ...
          <Image
            source={imagepath.mangopay}
            style={{width: 100, height: 20, tintColor: colors.primaryTextColor}}
            resizeMode="contain"
          />
        </View>
